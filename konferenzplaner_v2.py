...
def erstelle_konferenzplan(df_lehrer_filtered, klassenlehrer_map_full, plan_nummer, startzeit=14):
    ...
    max_slots = 15  # Erhöht für mehr Flexibilität
    total_slots = min(max_slots, math.ceil(total_klassen / 3) + 3)  # Puffer für Optimierung
    zeitslots = list(range(total_slots))

    if total_klassen > max_slots * 3:
        st.error(f"Zu viele Klassen ({total_klassen}) für Plan {plan_nummer}. Maximal möglich: {max_slots * 3}.")
        return None, None, None, None, None
    ...
    raum_var = {k: model.NewIntVar(0, 2, f"raum_{k}") for k in klassen}
    ...
    for s in zeitslots:
        ...
        model.Add(sum(klassen_in_slot) <= 3)  # Lockerer: <=3 für alle Slots
    ...
    # NEU: Wartezeit-Optimierung - Constraint für max Wartezeit <=60 min
    spread_vars = []
    for lehrer, kl_liste in lehrer_map.items():
        if len(kl_liste) > 0:
            kl_liste = list(kl_liste)
            min_slot = model.NewIntVar(0, total_slots - 1, f"min_slot_{lehrer}")
            max_slot = model.NewIntVar(0, total_slots - 1, f"max_slot_{lehrer}")
            model.AddMinEquality(min_slot, [slot_var[k] for k in kl_liste])
            model.AddMaxEquality(max_slot, [slot_var[k] for k in kl_liste])
            spread = model.NewIntVar(0, total_slots, f"spread_{lehrer}")
            model.Add(spread == max_slot - min_slot + 1)
            n = len(kl_liste)
            model.Add(spread <= n + 3)  # Warte <=60 min: (spread - n)*20 <=60 => spread <= n+3
            spread_vars.append(spread)

    # Objective: Minimiere Konflikte + Spreads (Wartezeiten)
    model.Minimize(sum(konfliktvars) + 10 * sum(spread_vars))  # Gewichte Spread höher
    ...
    solver.parameters.max_time_in_seconds = 180  # Erhöht für komplexere Optimierung
    ...
    if status in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
        zeitschema = [f"{startzeit + (i * 20) // 60}:{(i * 20) % 60:02d}" for i in range(total_slots)]
        plan, lehrer_einsaetze, klassensicht = [], {l: [] for l in lehrer_map}, {k: [] for k in klassen}

        for k in klassen:
            slot = solver.Value(slot_var[k])
            raum = solver.Value(raum_var[k])
            zeit = zeitschema[slot]
            teilnehmer = [l for l, ks in lehrer_map.items() if k in ks]
            for l in teilnehmer:
                lehrer_einsaetze[l].append((zeit, k, f"Raum {raum+1}"))
                klassensicht[k].append(l)
            plan.append((k, zeit, f"Raum {raum+1}", ", ".join(sorted(teilnehmer))))

        df_plan = pd.DataFrame(plan, columns=["Klasse", "Uhrzeit", "Raum", "Lehrkräfte"])
        df_plan = df_plan.sort_values(by="Uhrzeit")
        ...
        for l, einsaetze in lehrer_einsaetze.items():
            if len(einsaetze) > 0:
                # Direkte Slot-Nummern verwenden (nicht Zeit-Index)
                slot_numbers = []
                for z, k, r in einsaetze:
                    try:
                        slot_index = zeitschema.index(z)
                        slot_numbers.append(slot_index)
                    except ValueError:
                        # Fallback: Slot aus Zeit berechnen
                        slot_index = int(z.split(":")[0]) * 3 + int(z.split(":")[1]) // 20
                        slot_numbers.append(slot_index)
                
                if slot_numbers:
                    min_slot = min(slot_numbers)
                    max_slot = max(slot_numbers)
                    # Sicherstellen, dass Berechnung immer positiv ist
                    dauer = max(0, (max_slot - min_slot + 1) * 20)  # Minimum 0
                    anwesenheit = len(einsaetze) * 20
                    leerlauf = max(0, dauer - anwesenheit)  # Minimum 0
                else:
...
if lehrer_file and klassenlehrer_file:
    ...
    try:
        ...
        for _, row in df_klassenlehrer.iterrows():
            ...
            lehrer1 = str(row[klassenlehrer_cols[1]]).strip()
            lehrer2 = str(row[klassenlehrer_cols[2]]).strip() if len(klassenlehrer_cols) > 2 and pd.notna(row[klassenlehrer_cols[2]]) and str(row[klassenlehrer_cols[2]]).strip() != '' else None
            
            klassenlehrer_map[klasse] = lehrer1
            klassenlehrer_map_full[klasse] = [lehrer1]
            if lehrer2:
                klassenlehrer_map_full[klasse].append(lehrer2)
        
        st.session_state.klassenlehrer_map = klassenlehrer_map
        st.session_state.klassenlehrer_map_full = klassenlehrer_map_full
        
        # Manuelle Zuordnung basierend auf Ihrer Klassenlehrer-Datei (nur Sek1)
        sek1_klassen = ['5a', '5b', '5c', '5d', '5e', '6a', '6b', '6c', '6d', '7a', '7b', '7c', '7d', '8a', '8b', '8c', '8d', '9a', '9b', '9c', '9d']
        ...
        df_lehrer_1 = filter_klassen_direct(df_lehrer, sek1_klassen)

        # Debug-Ausgabe zur Überprüfung (nur für Plan 1)
        klassen_1 = sorted(df_lehrer_1["Klasse"].unique())

        st.write(f"**Plan 1 (Sek I - Tag 1):** {len(klassen_1)} Klassen")
        if klassen_1:
        ...
        if st.button("🚀 Konferenzplan erzeugen"):
            # Nur Plan 1 erstellen (Tag 1 - Sek I)
            with st.spinner("Erstelle Plan 1 (Sekundarstufe I - Tag 1)..."):
                (st.session_state.df_plan_1, st.session_state.df_warte_1, 
                 ...
                 st.session_state.statistik_1) = erstelle_konferenzplan(df_lehrer_1, klassenlehrer_map_full, 1, 14)
...
if st.session_state.berechnung_abgeschlossen:
    ...
    if st.session_state.df_plan_1 is not None:
        ...
        with st.expander("📋 **Konferenzplan** - Sekundarstufe I (Tag 1)", expanded=True):
    ...
    def export_plan_to_pdf(df_plan, lehrer_einsaetze, df_klassensicht, klassenlehrer_map_full, plan_name):
        pdf_buffer = BytesIO()
        ...
        # Zeit-Raum-Matrix
        zeitslots = sorted(df_plan["Uhrzeit"].unique(),
        ...
        col_widths = [60]
        for i in range(3):
            col_widths.append((available_width - 60) / 3)
        
        matrix_data = [
            [Paragraph("Uhrzeit", styles["Normal"]), 
             Paragraph("Raum 1", styles["Normal"]), 
             Paragraph("Raum 2", styles["Normal"]), 
             Paragraph("Raum 3", styles["Normal"])]
        ]
        
        for slot in zeitslots:
            row = [Paragraph(slot, styles["Normal"]), "", "", ""]
            df_slot = df_plan[df_plan["Uhrzeit"] == slot]
            
            for _, rowdata in df_slot.iterrows():
                r = int(rowdata["Raum"].split()[-1]) - 1
                ...
                if is_klassenlehrer:
                    row[r + 1] = Paragraph(f"<b>{cell_text}</b>", styles["Normal"])
                else:
                    row[r + 1] = Paragraph(cell_text, styles["Normal"])
        ...
        slot_width = (available_width_teacher - 70) / len(zeitslots)
        for _ in zeitslots:
    ...
    if st.session_state.df_plan_1 is not None:
        ...
        st.download_button("📥 Excel", excel_data_1, "konferenzplan_sek1.xlsx", use_container_width=True)
        ...
        if st.button("📄 PDF erstellen", use_container_width=True, key="pdf1"):
            with st.spinner("PDF wird erstellt..."):
                ...
                st.download_button("📄 PDF herunterladen", pdf_data_1, "konferenzplan_sek1.pdf", use_container_width=True)
